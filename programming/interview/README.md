...
- паузы GC
- хипдамп
- гарантировать порядок обхода в мапе (linkedhashmap)
- какие минусы у JVM
- минусы Java
- функциональные или нефункциональные - в чем отличия (бизнес - одно, а нефункциональные - это выдерживать нагрузку, технические требования)
- как избавиться от циклической зависимости
- Для чего keepalive (чтобы держать поток открытым) в https
- Для его нужен OpenAPI
- Что делает docker pull
- Чем отличаются под и deployment в кубере
- Как связаны topic и partition
- Для чего нужен ключ партиционирования
- Какие гарантии (целостности) внешние ключи
- По умолчанию индексы на unique contsraints создаются
- Знаком ли с конкретными prometheus метриками

- curcuit breaker
- Как можно обновлять контекст
- Выделение памяти jvm, флаг heapdump
- Не хватает памяти кубера при выделении в jvm
- Кубер может не выделять память. Т.е. падает OOM даже когда выделено достаточно
- Какие преимущества оптимистичной блокировки над пессимистичной
- Частичный и покрывающий индексы
- Когда произойдет удаление (ключи транзакций)
- Можно ли принудительно удалить запись с диска
- Как можно запустить 4 запроса параллельно без CompleteableFuture
- В чем отличие процесса от потока. Что могут шерить потоки между собой, что не могут процессы (оперативка, heap java)
- Дженерики
- TreeSet и LinkedHasSet

- Маркерные интерфейсы
- JIT и его влияние
- Деоптимизация кода
- К чему может привести нехватка метаспейса в настройках jvm. Особенно в контексте GC

LinkedList/ArrayList что быстрее пробежать итератором
Пример MinStack в Java
...
![Соберет ли мусорщик мусор](img.png)

- Анонимные классы. Транзакции в анонимных классах ![img_1.png](img_1.png)
- Принципиальное отличие executorService и ForkJoinPool
- deadlock, livelock, thread starvation
- При создании анонимных классов, каждый раз
- Проблемы со сборкой мусора, потому что он не ходит в метаспейс, где лежит список классов. 
- Теория поколений
- LinkedList не любят, т.к. реализовано как объекты, разбросанные по хипу
- Дефрагментациия - данные в разных местах лежат по хипу

- Проблема Java - Future.get() реализовано как busy loop
- При проксировании запроса keepAlive через nginx на nginx можно установить длительность keepAlive соединения и сколько запросов можно передать серверу в рамках этого соединения
- Сокет - постоянно открытое соединение двунаправленное
